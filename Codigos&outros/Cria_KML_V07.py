...
# Criação da coluna Tipo Complet (Tipo_Comp)
dados["Tipo_Comp"] = dados.apply(
    lambda row: (
        row["TipoPlu"] if pd.isna(row["TipoFlu"]) else (
            row["TipoFlu"] if pd.isna(row["TipoPlu"]) else f"{row['TipoPlu']}_{row['TipoFlu']}"
        )
    ),
    axis=1
)
...
def gerar_kml(dados, nome_roteiro, local_saida, data_criacao):
    ...
    for _, row in dados.iterrows():
        estacao = row["Estacao"]
        rot = row["Rot"]
        has_P = not pd.isna(row["LatitudePLU"]) and not pd.isna(
            row["LongitudePLU"])
        has_F = not pd.isna(row["LatitudeFLU"]) and not pd.isna(
            row["LongitudeFLU"])
        ...
        if usar_ambos:
            dist = geodesic((row["LatitudePLU"], row["LongitudePLU"]),
                            (row["LatitudeFLU"], row["LongitudeFLU"])).meters
            if dist < 200:
                usar_P = False
                usar_F = True
                usar_ambos = False

        lat = row["LatitudePLU"] if usar_P else row["LatitudeFLU"]
        lon = row["LongitudePLU"] if usar_P else row["LongitudeFLU"]
...
"""
        ...
        for campo, valor in [
            ("Roteiro", rot), ("Tipo", tipo), ("Tipo_Comp", Tipo_Comp),
            ("Codigo", codigo),
