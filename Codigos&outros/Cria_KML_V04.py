...
local_entrada = r"Z:\GEHITE\Principal\0-Gerencial\MAPAS\2025\SHAPEFILES\Estacoes_2025 SHP"
...
def gerar_kml(dados, nome_roteiro, pasta_saida, data_criacao):
    ...
    ET.SubElement(label_style, "scale").text = "1"  # Nome visível no mapa
...
descricao = f"""Roteiro: {rot}<br>
Código: {codigo}<br>
Tipo: {tipo}<br>"""
ET.SubElement(placemark, "description").text = descricao

        ext = ET.SubElement(placemark, "ExtendedData")
        for campo, valor in [("Roteiro", rot), ("Tipo", tipo), ("Código", codigo), ("Latitude", lat), ("Longitude", lon)]:
    ...
    # Escrever o KML com a data no nome do arquivo
